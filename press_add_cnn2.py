...
rnd = tf.placeholder(tf.float32, [None, 2048])
...
weights = {
    ...
    'wc1': tf.Variable(tf.random_normal([8, 1, 1, 4]), name='press_wc1'),
    # 5x5 conv, 32 inputs, 64 outputs
    'wc2': tf.Variable(tf.random_normal([8, 1, 4, 8]), name='press_wc2'),
    # fully connected, 7*7*64 inputs, 1024 outputs
    'wd1': tf.Variable(tf.random_normal([32 * 8+2048, 1024]), name='press_wd1'),
    # 1024 inputs, 10 outputs (class prediction)
    'out': tf.Variable(tf.random_normal([1024, n_classes]), name='press_wout')
}

biases = {
    'bc1': tf.Variable(tf.random_normal([4]), name='press_bc1'),
    'bc2': tf.Variable(tf.random_normal([8]), name='press_bc2'),
...
def test(wav):
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, "data/press_add/press_add_cnn2.ckpt")
...
def train(mnist):
    ...
    with tf.Session() as sess:
        ...
        saver.save(sess, "data/press_add/press_add_cnn2.ckpt")
