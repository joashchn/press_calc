...
def read_data_sets(train_dir, fake_data=False, one_hot=False):
    ...
    for wav, label, fr in reader:
        ...
        wav_max = numpy.max(wav_arr, axis=0)
        wav_min = numpy.min(wav_arr, axis=0)
        # print(wav_max, wav_min)
        for i in range(len(wav_arr)):
            wav_arr[i] = (wav_arr[i] - wav_min) / (wav_max - wav_min)
        ...
        for i in range(0, 100):
            if float(fr) >= i and float(fr) < (i+1):
                label_arr.append(1)
            else:
                label_arr.append(0)
        label_list.append(label_arr)

        # label_list.append(float(fr))

    # print wav_list
    # print label_list
    print('There are %d wav\nThere are %d label' % (len(wav_list), len(label_list)))

    train_images = numpy.array(wav_list)
    train_images.reshape(-1, 512)
    train_labels = numpy.array(label_list)
    # label_max = numpy.max(train_labels, axis=0)
    # label_min = numpy.min(train_labels, axis=0)
    # print (label_max, label_min)
    # for i in range(len(train_labels)):
    #     train_labels[i] = (train_labels[i] - label_min) / (label_max - label_min)
    # train_labels = train_labels[:, numpy.newaxis]
    print (train_images)
    print (train_labels)
